...
```bash
venv/bin/python pytest --cov=/tests/
```
...
Our backend is deployed to Azure App Service using GitHub Actions. This automated CI/CD pipeline handles building and deploying the application whenever changes are pushed to the main branch.

#### Key Deployment Configuration

1. **GitHub Workflow**: The workflow file `.github/workflows/main_prima-pizza-backend-west.yml` contains the complete deployment configuration.

2. **Runtime Configuration**:
   - Python 3.10 runtime
   - Gunicorn with 4 workers and 600 second timeout
   - Port 8000

3. **Environment Variables**:
```
FLASK_APP=services/prima_pizza/app.py
FLASK_ENV=production
JWT_SECRET_KEY=your_jwt_secret_key
DATABASE_URL=your_mongodb_uri
```

4. **Authentication**: Azure App Service Authentication must be disabled to prevent platform-level auth from interfering with our app's JWT authentication.

5. **URL Structure**: The API endpoints use trailing slashes (e.g., `/api/v1/toppings/`) due to Flask's routing behavior.

6. **CORS Configuration**: The backend allows requests from all origins by using a simplified CORS configuration:
```python
CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=False)
```

### 7.2 Troubleshooting Backend Deployment

1. 401 Unauthorized Errors: If API endpoints return 401 errors even on public routes, check that Azure App Service Authentication is disabled using:
```bash
az webapp auth update --resource-group prima-pizza-rg-west --name prima-pizza-backend-west --enabled false
```

2. Log Access: Review logs through the Azure portal under "App Service Logs" or via Kudu console.

3. Oryx Build System: Azure uses the Oryx build system to automatically install dependencies. Monitor the build logs if dependencies aren't being correctly installed.

4. Dynamic Runtime Directory: The application runs in a dynamic directory specified by $APP_PATH, not /home/site/wwwroot.
